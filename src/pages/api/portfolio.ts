  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      await prisma.$transaction(async (tx) => {
        // Update Hero
        const existingHero = await tx.hero.findFirst()
        if (existingHero) {
          ...
          await tx.role.deleteMany({ where: { heroId: existingHero.id } })
        ...
        // Update About
        const existingAbout = await tx.about.findFirst()
        if (existingAbout) {
        ...
        await tx.featuredProject.deleteMany()
        await tx.featuredProject.createMany({
        ...
        await tx.otherProject.deleteMany()
        await tx.otherProject.createMany({
        ...
        // Update Social
        const existingSocial = await tx.social.findFirst()
        if (existingSocial) {
      ...
      res.status(500).json({ error: 'Failed to update data', details: errorMessage })
    }
  } else {
